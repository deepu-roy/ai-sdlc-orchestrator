...
allowed-tools: Read, Write, Bash(git:*), Bash(curl:*), Bash(yq:*), Bash(jq:*), Bash(.claude/scripts/*), mcp__chrome__navigate, mcp__chrome__screenshot, mcp__chrome__click, mcp__chrome__type, mcp__chrome__find, mcp__chrome__get_page_text, mcp__chrome__read_console_messages, mcp__chrome__read_network_requests, mcp__chrome__form_input
---
...
1. **Load project context first.** Read `.claude/project/PROFILE.md`, `.claude/project/CLAUDE.md`, `.claude/project/overrides/verify-story.md`, and `.claude/project/guidelines/*`. Abort if PROFILE.md missing.
...
4. **Never enter real credentials.** Use only test accounts from `.claude/project/overrides/verify-story.md`. If credentials missing, return BLOCKED for that AC.
...
- `.claude/project/CLAUDE.md` (if present)
- `.claude/project/overrides/verify-story.md` (if present)
...
```bash
HEALTH_FRONTEND=$(yq -r '.gates.healthcheck.frontend // "n/a"' .claude/project/PROFILE.md)
HEALTH_BACKEND=$(yq -r '.gates.healthcheck.backend // "n/a"' .claude/project/PROFILE.md)
START_FRONTEND=$(yq -r '.gates.startup.frontend // "n/a"' .claude/project/PROFILE.md)
START_BACKEND=$(yq -r '.gates.startup.backend // "n/a"' .claude/project/PROFILE.md)
WAIT_SECS=$(yq -r '.gates.startup_wait_seconds // 15' .claude/project/PROFILE.md)
```

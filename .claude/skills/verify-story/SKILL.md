...
```bash
HEALTH_CMD=$(yq -r '.gates.healthcheck.frontend // "n/a"' .claude/project/PROFILE.md)
...
if ! eval "$HEALTH_CMD"; then
  ...
  yq -r '.gates.startup.frontend' .claude/project/PROFILE.md
  exit 1
fi
```
...
if [[ -n "$PR_ID" ]]; then
  .claude/scripts/ado-pr-comment.sh "$PR_ID" active \

...
```bash
.claude/scripts/ado-wi-show.sh $1 | jq '.comments // .fields["System.History"]'
```
...
- Move to a new §7 Deferred decisions with rationale

For each still UNANSWERED question:
- Leave in §6, flag as [BLOCKING] if it prevents technical design,

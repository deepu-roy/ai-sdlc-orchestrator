...
Read `gates.compile.<layer>` from `.github/project/PROFILE.md`
where `<layer>` matches this slice's declared layer.
Skip if value is `n/a`.

```bash
COMPILE_CMD=$(yq -r '.gates.compile.<layer>' .github/project/PROFILE.md)
...
```bash
STARTUP_CMD=$(yq -r '.gates.startup.<layer>' .github/project/PROFILE.md)
HEALTH_CMD=$(yq -r '.gates.healthcheck.<layer>' .github/project/PROFILE.md)
WAIT=$(yq -r '.gates.startup_wait_seconds // 15' .github/project/PROFILE.md)
